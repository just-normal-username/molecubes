...
void send_movement_command(float target_rad, float speed, float acc, float jerk) {
    Payload p = {};
    p.payload_servo.radians = target_rad;
    p.payload_servo.speed = speed;
    p.payload_servo.acceleration = acc;
    p.payload_servo.jerk = jerk;
